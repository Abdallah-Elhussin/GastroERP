...
@Component({
  ...
  templateUrl: './purchase-invoices.page.html',
  styleUrl: './purchase-invoices.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PurchaseInvoicesPage implements OnInit {
  ...
  breadcrumbs = [
    ...
    { labelKey: 'pur.nav.purchaseInvoices' }
  ];
  ...
  edit(): void {
    ...
    if (!row) return;
    void this.router.navigate(['/purchases/purchase-invoices', row.id]);
  }
  ...
  cancel(): void {
    const row = this.selected();
    if (!row || !this.canManage()) return;
    if (!confirm(this.t('pur.pi.confirmCancel'))) return;
  ...
  paymentModeLabel(row: PurchaseInvoiceDoc): string {
    switch (Number(row.paymentMode)) {
      case 1:
        return this.t('pur.pi.paymentMode.credit');
      case 2:
        return this.t('pur.pi.paymentMode.cash');
      default:
        return '—';
    }
  }
  ...
  private runAction(action: () => import('rxjs').Observable<void>): void {
    ...
    action().subscribe({
      ...
      error: err => {
        this.error.set(err?.error?.error ?? this.t('pur.pi.actionFailed'));
        this.acting.set(false);
      }
    });
  }
