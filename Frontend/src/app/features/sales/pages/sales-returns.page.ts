...
import { BackOfficeSalesReturn } from '../../../core/models/back-office-sales-return.models';
...
@Component({
  selector: 'app-sales-returns-page',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, MatIconModule],
  templateUrl: './sales-returns.page.html',
  styleUrl: './sales-returns.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SalesReturnsPage implements OnInit {
  ...
  rows = signal<BackOfficeSalesReturn[]>([]);
  selectedId = signal<string | null>(null);
  search = signal('');
  statusFilter = signal<StatusFilter>('all');
  invoiceNumbers = signal<Record<string, string>>({});
  ...
  ngOnInit(): void {
    this.invoiceRepo
      ...
      .subscribe(invoices => {
        ...
        for (const inv of invoices) map[inv.id] = inv.invoiceNumber;
        this.invoiceNumbers.set(map);
      });
    this.load();
  }
  ...
  invoiceNumber(invoiceId: string): string {
    return this.invoiceNumbers()[invoiceId] ?? invoiceId.slice(0, 8);
  }
  ...
  select(row: BackOfficeSalesReturn): void {
    this.selectedId.set(row.id);
  }

  createNew(): void {
    void this.router.navigate(['/sales/returns/new']);
  }

  edit(): void {
    const row = this.selected();
    if (!row) return;
    void this.router.navigate(['/sales/returns', row.id]);
  }
  ...
  statusName(row: BackOfficeSalesReturn): string {
  ...
  statusLabel(row: BackOfficeSalesReturn): string {
    switch (this.statusName(row)) {
      case 'Approved':
        return this.t('bos.ret.status.approved');
      case 'Posted':
        return this.t('bos.ret.status.posted');
      case 'Cancelled':
        return this.t('bos.ret.status.cancelled');
      default:
        return this.t('bos.ret.status.draft');
    }
  }
