...
import { BackOfficeSalesDebitNote } from '../../../core/models/back-office-sales-debit-note.models';
...
@Component({
  selector: 'app-sales-debit-notes-page',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, MatIconModule],
  templateUrl: './sales-debit-notes.page.html',
  styleUrl: './sales-debit-notes.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SalesDebitNotesPage implements OnInit {
  ...
  rows = signal<BackOfficeSalesDebitNote[]>([]);
  ...
  select(row: BackOfficeSalesDebitNote): void {
    this.selectedId.set(row.id);
  }

  createNew(): void {
    void this.router.navigate(['/sales/debit-notes/new']);
  }

  edit(): void {
    const row = this.selected();
    if (!row) return;
    void this.router.navigate(['/sales/debit-notes', row.id]);
  }
  ...
  statusName(row: BackOfficeSalesDebitNote): string {
  ...
  statusLabel(row: BackOfficeSalesDebitNote): string {
    switch (this.statusName(row)) {
      case 'Approved':
        return this.t('bos.dnte.status.approved');
      case 'Posted':
        return this.t('bos.dnte.status.posted');
      case 'Cancelled':
        return this.t('bos.dnte.status.cancelled');
      default:
        return this.t('bos.dnte.status.draft');
    }
  }
