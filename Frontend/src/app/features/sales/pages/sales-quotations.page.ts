...
import { BackOfficeSalesQuotation } from '../../../core/models/back-office-sales-quotation.models';
...
@Component({
  selector: 'app-sales-quotations-page',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, MatIconModule],
  templateUrl: './sales-quotations.page.html',
  styleUrl: './sales-quotations.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SalesQuotationsPage implements OnInit {
  ...
  rows = signal<BackOfficeSalesQuotation[]>([]);
  ...
  select(row: BackOfficeSalesQuotation): void {
  ...
  edit(): void {
    ...
    if (!row) return;
    void this.router.navigate(['/sales/quotations', row.id]);
  }
  ...
  statusName(row: BackOfficeSalesQuotation): string {
  ...
  statusLabel(row: BackOfficeSalesQuotation): string {
    switch (this.statusName(row)) {
      case 'Approved':
        return this.t('bos.quo.status.approved');
      case 'Posted':
        return this.t('bos.quo.status.posted');
      case 'Cancelled':
        return this.t('bos.quo.status.cancelled');
      default:
        return this.t('bos.quo.status.draft');
    }
  }
